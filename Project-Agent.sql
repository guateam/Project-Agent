/*
 ...
 Source Schema         : project-agent

 Target Server Type    : MySQL
 Target Server Version : 50553
 File Encoding         : 65001

 Date: 18/12/2018 14:55:24
*/
...
) ENGINE = InnoDB AUTO_INCREMENT = 4 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '回答评论表' ROW_FORMAT = Compact;

-- ----------------------------
-- Records of answercomments
-- ----------------------------
INSERT INTO `answercomments` VALUES (1, 1, '这是评论这是评论这是评论这是评论这是评论这是评论这是评论这是评论这是评论这是评论这是评论这是评论这是评论这是评论这是评论这是评论这是评论这是评论这是评论这是评论', 0, '2018-12-15 18:58:24', 1);
...
CREATE TABLE `answers`  (
  ...
  `content` text CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '答案内容',
  `agree` int(11) NOT NULL COMMENT '赞同数',
...
) ENGINE = InnoDB AUTO_INCREMENT = 4 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '答案' ROW_FORMAT = Compact;
...
CREATE TABLE `article`  (
  ...
  `userID` int(255) NULL DEFAULT NULL COMMENT '发布文章的用户ID',
  `content` text CHARACTER SET utf8 COLLATE utf8_general_ci NULL COMMENT '文章内容',
  `edittime` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '最后一次编辑时间',
...
-- Records of article
-- ----------------------------
INSERT INTO `article` VALUES (3, 1, '修改修改修改', '2018-12-18 14:53:44');
...
) ENGINE = InnoDB AUTO_INCREMENT = 2 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '收藏：用户-答案映射' ROW_FORMAT = Compact;
...
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '收藏：用户-文章映射' ROW_FORMAT = Compact;
...
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '用户-专栏 关注关系映射表' ROW_FORMAT = Compact;
...
) ENGINE = InnoDB AUTO_INCREMENT = 2 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '用户-话题 关注关系映射表' ROW_FORMAT = Compact;
...
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '用户-用户 关注关系映射表' ROW_FORMAT = Compact;
...
CREATE TABLE `questioncomments`  (
  ...
  `agree` int(11) NOT NULL COMMENT '赞同数\n',
  `createtime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '创建时间',
  `questionID` int(11) NULL DEFAULT NULL COMMENT '对应问题ID\n',
  PRIMARY KEY (`qcommentID`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '问题评论' ROW_FORMAT = Compact;
...
CREATE TABLE `questionlog`  (
  ...
  `reason` varchar(200) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '编辑理由',
  PRIMARY KEY (`idQuestionLog`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '问题日志' ROW_FORMAT = Compact;
...
CREATE TABLE `questions`  (
  ...
  `edittime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '最后编辑时间\n',
  `userID` int(11) NOT NULL COMMENT '创建者ID',
  `tags` varchar(45) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '问题标签',
  PRIMARY KEY (`questionID`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 6 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '问题\r\n' ROW_FORMAT = Compact;

-- ----------------------------
-- Records of questions
-- ----------------------------
INSERT INTO `questions` VALUES (1, '刚刚研制成功的世界首台分辨力最高紫外超分辨光刻装备意味着什么？对国内芯片行业有何影响？', '这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述这里是描述', '2017-01-06 00:00:00', 1, NULL);
INSERT INTO `questions` VALUES (2, '程序员是如何看待「祖传代码」的？', '说明一下……此处「祖传代码」与百度搜索「祖传代码」词条出现的某直播平台后缀无关……\n\n几位答主提到祖传代码不仅游戏领域有，其他领域也存在，所以删除了游戏标签，问题修改为作为程序员对于「祖传代码」这种说法或其本身有什么看法吐槽。\n\n对码农一无所知，提问的初衷大概是为了爱与和平。\n\n感谢扫盲_(:з」∠)_', '2018-12-15 18:46:54', 1, NULL);
INSERT INTO `questions` VALUES (3, '为什么总是有人说 Java 啰嗦，却没人说 C++ 啰嗦？', '哈哈哈哈哈哈', '2018-12-15 18:47:38', 1, NULL);
INSERT INTO `questions` VALUES (4, '有哪些让你目瞪口呆的 bug？', '', '2018-12-15 18:49:52', 1, NULL);
INSERT INTO `questions` VALUES (5, '互联网行业的裁员潮是否已经开始了？', '【此为2018年的提问】\n\n看到媒体也开始说这件事了……普通员工如何扛过去呢？', '2018-12-15 18:51:16', 1, NULL);
...
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '用户行为表\n' ROW_FORMAT = Compact;
...
CREATE TABLE `users`  (
  ...
  `email` varchar(45) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '用户名/邮箱',
  ...
  `headportrait` text CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '头像',
  ...
  `phonenumber` varchar(45) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '电话号码',
...
-- Records of users
-- ----------------------------
INSERT INTO `users` VALUES (1, 'zhangyu199946@126.com', '拉拉人', 'ec847003d2eadc9baf60853e8391e167a292c21f01892fcb8bad0f4af6cd74a7', '', 0, 0, '23zK0V1dIgLU5PEQ6vcmxMwSp', NULL, NULL, NULL);
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `answersinfo` AS select `answers`.`answerID` AS `answerID`,`answers`.`userID` AS `userID`,`answers`.`edittime` AS `edittime`,`answers`.`content` AS `content`,`answers`.`agree` AS `agree`,`answers`.`disagree` AS `disagree`,`answers`.`answertype` AS `answertype`,`answers`.`questionID` AS `questionID`,`users`.`nickname` AS `nickname`,`users`.`headportrait` AS `headportrait` from (`answers` join `users`) where (`answers`.`userID` = `users`.`userID`) order by `answers`.`agree` desc;
